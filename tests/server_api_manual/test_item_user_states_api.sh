...
main() {
    echo -e "${BLUE}==========================================${NC}" >&2
    echo -e "${BLUE}Тестирование API истории состояний элементов${NC}" >&2
    echo -e "${BLUE}==========================================${NC}" >&2
    ...
    WORKFLOW_ID=$(create_workflow "$TOKEN" "Test Workflow for ItemUserState")
    ...
    # Создаём проект
    PROJECT_ID=$(create_project "$TOKEN" "Test Project for ItemUserState")
    ...
    # ============================================================
    # Тест 1: Создание записи истории состояния
    # ============================================================
    echo "" >&2
    print_header "Тест 1: Создание записи истории состояния"
    
    STATE_DATA="{\"stateId\": ${STATE1_ID}, \"comment\": \"Первый переход\"}"
    RESPONSE=$(do_post "/api/v1/items/${ITEM_ID}/user-states" "$TOKEN" "$STATE_DATA")
    
    if is_success "$RESPONSE"; then
        RECORD_ID=$(echo "$RESPONSE" | jq -r '.id')
        if [ -n "$RECORD_ID" ] && [ "$RECORD_ID" != "null" ]; then
            print_success "Запись истории состояния создана с ID: $RECORD_ID"
        else
            print_error "Не удалось получить ID записи: $RESPONSE"
            RECORD_ID=""
        fi
    else
        print_error "Не удалось создать запись истории состояния: $RESPONSE"
        RECORD_ID=""
    fi
    ...
    if [ -n "$RECORD_ID" ]; then
        echo "" >&2
        print_header "Тест 2: Получение записи истории состояния"
        
        RESPONSE=$(do_get "/api/v1/items/user-states/${RECORD_ID}" "$TOKEN")
        FOUND_ID=$(echo "$RESPONSE" | jq -r '.id // empty')
        
        if [ "$FOUND_ID" = "$RECORD_ID" ]; then
            print_success "Запись истории состояния получена"
            COMMENT=$(echo "$RESPONSE" | jq -r '.comment // empty')
            print_header "Комментарий: $COMMENT"
        else
            print_error "Не удалось получить запись истории состояния: $RESPONSE"
        fi
        
        # Тест 3: Получение последней записи
        echo "" >&2
        print_header "Тест 3: Получение последней записи истории состояния"
        
        RESPONSE=$(do_get "/api/v1/items/${ITEM_ID}/user-states/last" "$TOKEN")
        LAST_ID=$(echo "$RESPONSE" | jq -r '.id // empty')
        
        if [ "$LAST_ID" = "$RECORD_ID" ]; then
            print_success "Последняя запись получена"
        else
            print_error "Последняя запись: $LAST_ID (ожидалось: $RECORD_ID)"
        fi
        
        # Тест 4: Создание второй записи
        echo "" >&2
        print_header "Тест 4: Создание второй записи истории состояния"
        
        STATE_DATA2="{\"stateId\": ${STATE2_ID}, \"comment\": \"Второй переход\"}"
        RESPONSE=$(do_post "/api/v1/items/${ITEM_ID}/user-states" "$TOKEN" "$STATE_DATA2")
        
        if is_success "$RESPONSE"; then
            RECORD_ID2=$(echo "$RESPONSE" | jq -r '.id')
            if [ -n "$RECORD_ID2" ] && [ "$RECORD_ID2" != "null" ]; then
                print_success "Вторая запись создана с ID: $RECORD_ID2"
                
                # Тест 5: Удаление второй записи
                echo "" >&2
                print_header "Тест 5: Удаление второй записи"
                
                do_delete "/api/v1/items/user-states/${RECORD_ID2}" "$TOKEN"
                
                RESPONSE=$(do_get "/api/v1/items/user-states/${RECORD_ID2}" "$TOKEN")
                CHECK_ID=$(echo "$RESPONSE" | jq -r '.id // empty')
                
                if [ -z "$CHECK_ID" ]; then
                    print_success "Запись успешно удалена"
                else
                    print_error "Не удалось удалить запись"
                fi
            else
                print_error "Не удалось создать вторую запись: $RESPONSE"
            fi
        else
            print_error "Не удалось создать вторую запись: $RESPONSE"
        fi
        
        # Тест 6: Получение списка записей
        echo "" >&2
        print_header "Тест 6: Получение списка записей истории состояний"
        
        RESPONSE=$(do_get "/api/v1/items/user-states?itemId=${ITEM_ID}" "$TOKEN")
